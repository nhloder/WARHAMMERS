...
module.exports = {
  register: async (req,res) => {
    ...
    const {email, password, username, about, profile_pic, is_admin} = req.body

    const result = await db.hash.find_user(email)
    if (result[0]) return res.status(409).send({message: 'E-mail already in use.'})
    ...
    const createdUser = await db.hash.add_to_user({
      email, hash_id, username, about, profile_pic, is_admin
    })

    req.session.user = {id: createdUser[0].user_id,
